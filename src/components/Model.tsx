...
export default function Model(props: ModelProps) {
  ...
  const modelReplaced = useRef(false);
  const [suzanneGeometry, setSuzanneGeometry] = useState(
    nodes.Suzanne.geometry
  );

  const [suzanneMaterial, setSuzanneMaterial] = useState(materials.Suzane);
    ...
    if (actions["CameraAction"] && position && rotation) {
      ...
      for (let i = 0; i < position.times.length - 1; i++) {
        ...
        if (time >= rotation.times[i] && time < rotation.times[i + 1]) {
          ...
          // state.camera.quaternion.slerp(rotation.values[rotation.values.length - 1], 0.05);
        }
      }
    }

    let suzane = group?.current?.children.at(4);

    if (suzane && !modelReplaced.current) {
      if (isObjectInViewport(state.camera, suzane)) {
        console.log("suzane in view");
        modelReplaced.current = true;
        setSuzanneGeometry(nodes.Cube.geometry);
        setSuzanneMaterial(materials.Cube);
      }
    }
  });
